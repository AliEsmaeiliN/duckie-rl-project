...
MODEL_WITH_JERK_PATH = os.path.expanduser('~/workspace/rl_models/sac_vr1.cleanrl_model')
MODEL_WITHOUT_JERK_PATH = os.path.expanduser('~/workspace/rl_models/td3_vr1.cleanrl_model')


def load_actor(model_path):
    """Safely extracts state dict from CleanRL checkpoint framework."""
    is_downscaled = "ds" in model_path.lower()
    if "td3" in model_path.lower():
            actor = TD3Actor(downscaled=is_downscaled).to(DEVICE)
    else:
            actor = SACActor(downscaled=is_downscaled).to(DEVICE)
...
def run_evaluation_loop(actor, direction_string, is_downsampled):
...
if __name__ == "__main__":
    ...
    for model_label, actor_net in models.items():
        results[model_label] = {}
        is_downsampled = "ds" in model_label
        for d in directions:
            print(f"Evaluating model variant [{model_label}] navigating profile [{d}]...")
            results[model_label][d] = run_evaluation_loop(actor_net, d, is_downsampled)
    ...
    for col_idx, model_label in enumerate(model_keys):
        for d, color, style in zip(directions, ['#1f77b4', '#d62728'], ['-', '--']):
            ...
            axes[0, col_idx].set_ylim(-0.1, 1.1)
            ...
            axes[1, col_idx].set_ylim(-1.1, 1.1)
            axes[1, col_idx].set_ylabel("Steering ($\omega$)", fontsize=16)
            axes[1, col_idx].set_xlabel("Simulation Decision Steps", fontsize=14)
